...
mod Foo {
    ...
    #[no_mangle] // Prevent symbol mangling, so this function can be called externally.
    fn my_demo_function(a: u32) -> u32 {
        a
    }

    // Create an alias for `my_demo_function` with the exact symbol name.
    #[no_mangle]
    #[link_name = "my_demo_function"] // Link the alias to the same function.
    fn my_demo_function_alias(a: u32) -> u32 {
        a
    }
}
